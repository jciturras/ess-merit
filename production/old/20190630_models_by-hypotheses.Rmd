...
```{r MODELO 6 OCUPACION,include=FALSE, cache=TRUE}
elsoc_gini$sexo <- as.numeric(elsoc_gini$sexo)
sem.isco<-'
salirad =~meduc+mambi+mtdur
recompe =~resfu+rinte
...
'
fit.sem.isco <-sem(sem.isco,data=elsoc_gini,ordered=c("meduc","mambi","mtdur","resfu","rinte"))
...
fit.sem.egp <-sem(sem.egp07,data=elsoc_gini,ordered=c("meduc","mambi","mtdur","resfu","rinte"))
...
```{r MODELO 61 ESSpropio + EDUCACION + INGRESOS + OCUPACION + CONTROLES,include=FALSE, cache=TRUE}
sem.ess1.obj<-'
salirad =~meduc+mambi+mtdur
recompe =~resfu+rinte
salirad~ess+
...
fit.sem <-sem(sem,data=elsoc_gini,ordered=c("meduc","mambi","mtdur","resfu","rinte"))
fit.sem <-sem(sem,data=elsoc_gini)
...
med1sa<- med1[c(9:12,643:646,647:650,659:662),]  #salir adelante
med1re<- med1[c(41:44,651:654,655:658,663:666),] #Recompensa 
```
...
rmarkdown::render(input = here("code","20190630_models_by-hypotheses.rmd"),
                  output_file = here("code","20190630_models_by-hypothesesW18.pdf"), encoding = "UTF-8")
